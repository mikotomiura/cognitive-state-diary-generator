...
- **前日接続 (prev_day_ending)** -- Day 2以降で前日の末尾段落を Generator プロンプトに注入し、1文目で前日の出来事・感情・人物に具体的に言及させる。テーマレベルではなく出来事レベルの接続を実現
- **未解決フック** -- Day 1〜6 の末尾 2 文以内に「未解決の種」(未回収の出来事、予告的行動、解決されていない違和感、伏線的具体物等) の埋め込みを指示。余韻（文章を閉じる）とフック（文章を開く）の区別を明示
- **hook_strength 診断** -- Critic が 0.0〜1.0 でフック強度を評価し generation_log.json に記録 (Pass/Reject 判定には不使用)
