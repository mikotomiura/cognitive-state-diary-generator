...
class PipelineRunner:
    ...
    async def run(
        ...
        for event in events:
            day = event.day
            try:
                record = await self.run_single_day(event, current_state, day)
        ...
        return PipelineLog(
            ...
            prompt_hashes={},  # TODO: prompts/ のハッシュ計算を実装する (MVP では空)
            records=records,
    ...
    async def run_single_day(
        ...
        for attempt in range(self._config.max_retries):
            try:
                curr_state = await self._actor.update_state(prev_state, event)
                break
