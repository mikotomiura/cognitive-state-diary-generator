...
Phase 3 評価
  ...
  │
  └─ Reject → Phase 2 再生成 (temperature=0.5, revision_instruction 注入)
       ↓ Reject
...
LLMプロバイダの切り替えに対応するため、LLM呼び出しを抽象化する。現在は Anthropic Claude を使用している。
...
| LLMプロバイダの切替 | Anthropic以外のLLMを使用 | `LLMClient` 抽象クラスの新規実装を追加 |
