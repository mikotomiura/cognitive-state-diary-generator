...
- **フック制約 (Day 1-6):** 末尾2文以内の未解決フック必須。リトライ時は `structural_violations` に基づき、
  具体的な修正例付きの強化版制約に自動置換される
...
Phase 2 生成直後に実行される軽量なルールベースチェック (11項目)。以下を検査し、違反メッセージのリストを返す:
1. 余韻パターン上限、2. 場面構造の連続使用、3. 場面構造の上限、4. 主題語 per-day 上限、
5. 書き出しパターン上限、6. 冒頭テキスト Day 間 trigram 重複、7. 余韻テキスト Day 間 trigram 重複、
8. 禁止余韻パターン、9. 本文フレーズの Day 間重複 (15文字以上)、10. 文字数の理想範囲 (450文字以下)、
11. 末尾フックの弱さ (Day 1-6: 弱い修辞疑問・感情の結論で閉じるフックを検出)。
Critic（Phase 3）が検査しない構造的多様性を補完する。構造違反がある場合はボーナス再試行を強制する。

### structural_violations（構造的制約違反リスト）
`_validate_structural_constraints()` が返す違反メッセージのリスト。
ボーナス再試行時に Actor の `generate_diary()` に渡され、`critical_constraints` セクションの
フック制約を通常版から強化版（具体的な修正例付き）に置換することで、リトライの実効性を高める。

### ボーナス再試行 (Bonus Retry)
Critic が Pass と判定した場合でも、構造的制約違反が検出された場合に実行される
追加の再試行（リトライ予算を消費しない1回限定）。
違反内容は `revision_instruction`（修正指示セクション）と `critical_constraints`（冒頭禁止事項セクション）の
両方に注入される二重注入 (Dual Injection) 方式で Generator に伝達される。
フック関連違反の場合は具体的な修正例 (予告的行動・未回収の出来事・未解決の違和感) が付与される。
